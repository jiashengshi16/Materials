...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "attach previous Gemini self-debug reports for the same material, "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "verifier/diagnostics.json contains status=success."
        ),
    )
    parser.add_argument(
        "--success-root",
    ...
    if args.target_success_runs is not None:
        dataset_materials = {path.name for path in args.dataset.iterdir() if path.is_dir()}
        context_materials = materials_with_self_debug_reports()
        candidate_materials = dataset_materials & context_materials
        if not candidate_materials:
            raise SystemExit(
                "No dataset materials have previous self-debug report pairs under "
                f"{SELF_DEBUG_REVIEWS_ROOT}"
            )
