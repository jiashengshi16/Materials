...
NEXT_RUN_CONTEXT_DIR = "next_run_context"
...
DOCKER_BUILDER_PRUNE_COMMAND = ["docker", "builder", "prune", "--force", "--all"]
DOCKER_SYSTEM_PRUNE_COMMAND = ["docker", "system", "prune", "--force", "--all"]
DOCKER_PRUNE_COMMANDS = [DOCKER_BUILDER_PRUNE_COMMAND, DOCKER_SYSTEM_PRUNE_COMMAND]
...
def scoped_self_debug_source_files(
    reports: list[tuple[str, Path, Path]],
    candidate_reports: dict[str, list[tuple[str, Path, Path]]] | None = None,
    *,
    target_material: str,
    ...
    """Return every required report with explicit target/report scope metadata."""
    candidate_reports = candidate_reports or {}
    records: list[dict[str, object]] = []
    ...
    def append_reports(
        report_material: str,
        reports_for_material: list[tuple[str, Path, Path]],
        *,
        requested_scope: str,
    ) -> None:
        same_as_target = report_material == target_material
        scope = "same_material" if same_as_target else requested_scope
        safe_material = safe_context_name(report_material)

        for case_name, md_path, json_path in reports_for_material:
            safe_case = safe_context_name(case_name)
            for file_kind, src_path in (
                ("self_debug_report_md", md_path),
                ("self_debug_report_json", json_path),
            ):
                resolved_source = src_path.resolve()
                if resolved_source in seen_source_files:
                    continue
                seen_source_files.add(resolved_source)

                if scope == "same_material":
                    rel_path = Path("raw") / "same_material" / safe_case / src_path.name
                else:
                    rel_path = (
                        Path("raw")
    ...
    if include_same_material_reports:
        append_reports(
            target_material,
            reports,
            requested_scope="same_material",
        )

    for candidate_material, reports_for_candidate in sorted(candidate_reports.items()):
        # A CSV self-pair is intentionally reclassified as same_material rather
        # than being mislabeled as a different chemically similar material.
        append_reports(
            candidate_material,
            reports_for_candidate,
            requested_scope="candidate_material",
        )
...
def write_scoped_self_debug_context_bundle(
    target_root: Path,
    *,
    material: str,
    records: list[dict[str, object]],
    ...
    rendered: list[str] = [
        ...
        "## CRITICAL scope meanings",
        "",
        "- `same_material`: the report is from a prior run of the current target material itself. `report_material` equals `target_material`; this is not cross-material candidate context.",
        "- `candidate_material`: the report is from a different material selected as cross-material context. Do not blindly copy exact band counts, windows, projections, or final status.",
        "",
    ]

    if scopes == {"same_material"}:
        rendered.extend(
            [
                "All reports in this bundle have `scope: same_material`. They describe prior runs of the current target material itself. No candidate-material reports are included.",
                "",
            ]
        )
    else:
        rendered.extend(
            [
                "Before using any report below, identify its `scope`, `report_material`, `target_material`, and `same_as_target` value.",
                "",
            ]
        )
    ...
    for index, record in enumerate(records, start=1):
        ...
        rendered.extend(
            [
                ...
                "",
                f"- scope: `{record['scope']}`",
                f"- report_material: `{record['report_material']}`",
                f"- target_material: `{record['target_material']}`",
                f"- same_as_target: `{str(record['same_as_target']).lower()}`",
                f"- case: `{record['case']}`",
                f"- file_kind: `{record['file_kind']}`",
                f"- app_path: `{record['app_path']}`",
                f"- sha256: `{record['sha256']}`",
    ...
    index_payload = {
        ...
        "same_material_only": scopes == {"same_material"},
        "required_summary_path": "workflow/SELF_DEBUG_CONTEXT_SUMMARY.json",
        "required_bundle_path": "/app/self_debug_context/ALL_SELF_DEBUG_REPORTS.md",
        "records": records,
    }
...
def markdown_section(text: str, heading: str) -> str:
    """Return a top-level markdown section by exact heading text."""
...
def material_rows_from_executive_summary(text: str, material: str) -> str:
    section = markdown_section(text, "Executive summary")
    if not section:
        return ""
    rows = [
        line
        for line in section.splitlines()
        if line.startswith("| ") and (line.startswith("| Run ") or line.startswith("| ---") or f"| {material}," in line)
    ]
    if len(rows) <= 2:
        return ""
    return "\n".join(["## Executive summary rows for this material", "", *rows])


def material_per_run_sections(text: str, material: str) -> str:
    lines = text.splitlines()
    sections: list[str] = []
    index = 0
    while index < len(lines):
        ...
        if f"__{material}" in line or f"`{material}__" in line:
            sections.append(block)

    return "\n\n".join(sections)


def next_run_recommendation_for_material(material: str, diagnoses_path: Path) -> str:
    """Render the Codex next-run diagnosis file into material-specific context."""
    text = diagnoses_path.read_text(encoding="utf-8")
    pieces = [
        "# REQUIRED CODEX NEXT-RUN RECOMMENDATIONS",
        "",
        f"Target material: `{material}`",
        f"Source diagnosis file: `{diagnoses_path}`",
        "",
        "These are Codex-reviewed next-run recommendations, not raw self-debug reports.",
        "Use them as high-priority planning context, but still validate projection counts, kpoints, and energy windows from the current task files before running expensive commands.",
        "",
    ]

    executive_rows = material_rows_from_executive_summary(text, material)
    if executive_rows:
        pieces.extend([executive_rows, ""])

    for heading in ("Cross-run implementation rules", "Reference evidence from Gemini/older runs"):
        section = markdown_section(text, heading)
        if section:
            pieces.extend([section, ""])

    per_run = material_per_run_sections(text, material)
    if per_run:
        pieces.extend(["## Per-run Codex diagnoses for this material", "", per_run, ""])

    section = markdown_section(text, "Final acceptance checklist")
    if section:
        pieces.extend([section, ""])

    if not executive_rows and not per_run:
        return ""
    return "\n".join(pieces).rstrip() + "\n"


def write_next_run_context_bundle(
    target_root: Path,
    *,
    material: str,
    diagnoses_path: Path,
    ...
    bundle_root = target_root / NEXT_RUN_CONTEXT_DIR
    bundle_root.mkdir(parents=True, exist_ok=True)
    source_sha = sha256_file(diagnoses_path)

    raw_dir = bundle_root / "raw"
    raw_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy2(diagnoses_path, raw_dir / diagnoses_path.name)

    (bundle_root / "ALL_NEXT_RUN_RECOMMENDATIONS.md").write_text(
        recommendation_text,
        encoding="utf-8",
    )
    index_payload = {
        "target_material": material,
        "source_path": str(diagnoses_path),
        "source_sha256": source_sha,
        "required_bundle_path": "/app/next_run_context/ALL_NEXT_RUN_RECOMMENDATIONS.md",
        "raw_source_path": f"/app/next_run_context/raw/{diagnoses_path.name}",
    }
    (bundle_root / "index.json").write_text(
        json.dumps(index_payload, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
...
def next_run_context_instruction_appendix(material: str, has_next_run_context: bool) -> str:
    if not has_next_run_context:
        return ""

    return f"""

# Mandatory Codex Next-Run Recommendation Context

Before choosing the Wannierisation strategy, before writing the first `<seed>.win`,
and before creating `workflow/run_dir`, read the complete Codex next-run
recommendation bundle:

`/app/next_run_context/ALL_NEXT_RUN_RECOMMENDATIONS.md`

It is indexed at:

`/app/next_run_context/index.json`

This bundle contains curated diagnoses and next-run recommendations for
`{material}`. Treat these recommendations as high-priority planning context,
but the task instruction remains authoritative for material identity, `num_wann`,
`num_bands`, target-band handling, artifact contracts, and final status.

Hard requirements when using this context:
- Validate the kpoint mesh against the QE save tree before `pw2wannier90.x`.
- Validate projection count with `wannier90.x -pp` before expensive work.
- Derive final energy windows from the current `.eig` where available.
- Do not blindly copy reference windows or projections if the current task files contradict them.

After reading the bundle, and before any Wannier90/QE command, create:

`workflow/NEXT_RUN_CONTEXT_SUMMARY.json`
...
{{
  "target_material": "{material}",
  "bundle_path": "/app/next_run_context/ALL_NEXT_RUN_RECOMMENDATIONS.md",
  "index_path": "/app/next_run_context/index.json",
  "read_complete_bundle": true,
  "key_recommendations": [],
  "strategy_implications": [],
  "validation_checks_to_perform": []
}}
```

Hard gate: if `workflow/NEXT_RUN_CONTEXT_SUMMARY.json` is missing, invalid, or
claims `read_complete_bundle != true`, do not proceed. Return `status: "failed"`
and explain that the Codex next-run context preflight was incomplete.
"""
...
def scoped_context_instruction_appendix(
    material: str,
    reports: list[tuple[str, Path, Path]],
    candidate_reports: dict[str, list[tuple[str, Path, Path]]] | None = None,
    *,
    ...
    candidate_materials = sorted(candidate_reports)
    candidate_materials_other_than_target = [
        name for name in candidate_materials if name != material
    ]
    same_material_only = (
        not candidate_materials_other_than_target
        and (include_same_material_reports or material in candidate_materials)
    )

    if same_material_only:
        scope_contract = f"""
Every report in this bundle has `scope: same_material`. Each report is from a
prior run of the current target material `{material}` itself. For every record,
`report_material` must equal `target_material`, and `same_as_target` must be
true. These are not chemically similar candidate-material reports.
""".strip()
    else:
        scope_contract = """
This bundle may contain reports with different `scope` values:
- `same_material`: a prior run of the current target material itself.
- `candidate_material`: a report from a different material supplied as
  cross-material context.

Before using any report, identify its `scope`, `report_material`,
`target_material`, and `same_as_target` value. Candidate-material reports are
not reports for the target material.
""".strip()
...
{{
  ...
  "read_file_count": {expected_self_debug_file_count},
  "all_files_read": true,
  "scopes_present": ["same_material"],
  "same_material_only": true,
  "files": [
    {{
      "app_path": "/app/self_debug_context/raw/.../self_debug_report.md",
      "sha256": "...",
      "scope": "same_material or candidate_material",
      "report_material": "...",
      "target_material": "{material}",
      "same_as_target": true,
      "key_failure_or_lesson": "...",
...
def scoped_trace_verifier_script() -> str:
    return """#!/usr/bin/env python3
from __future__ import annotations
...
def verify(index_path: Path, summary_path: Path, trace_path: Path) -> list[str]:
    ...
    if summary.get(\"all_files_read\") is not True:
        errors.append(\"summary all_files_read is not true\")
    if summary.get(\"scopes_present\") != index.get(\"scopes_present\"):
        errors.append(
            f\"summary scopes_present={summary.get('scopes_present')!r} \"
            f\"does not match index scopes_present={index.get('scopes_present')!r}\"
        )
    if summary.get(\"same_material_only\") != index.get(\"same_material_only\"):
        errors.append(
            f\"summary same_material_only={summary.get('same_material_only')!r} \"
            f\"does not match index same_material_only={index.get('same_material_only')!r}\"
        )
    ...
    summary_by_identity = {
        (item.get(\"app_path\"), item.get(\"sha256\")): item
        for item in summary_files
        if isinstance(item, dict)
    }
    for record in index_records:
        ...
        if not app_path or not sha256:
            errors.append(f\"index record missing app_path or sha256: {record!r}\")
            continue
        summary_record = summary_by_identity.get((app_path, sha256))
        if summary_record is None:
            errors.append(f\"summary missing indexed file app_path/sha256: {app_path} {sha256}\")
            continue
        for field in (\"scope\", \"report_material\", \"target_material\", \"same_as_target\"):
            if summary_record.get(field) != record.get(field):
                errors.append(
                    f\"summary field mismatch for {app_path}: {field}=\"
                    f\"{summary_record.get(field)!r}, expected {record.get(field)!r}\"
                )
...
def write_trace_tools(
    environment_dir: Path,
    *,
    scoped_same_material_context: bool = False,
) -> None:
    ...
    verifier_text = (
        scoped_trace_verifier_script()
        if scoped_same_material_context
        else trace_verifier_script()
    )
...
def materialize_self_debug_context_dataset(
    ...
    *,
    ...
    next_run_diagnoses_path: Path | None = None,
    scoped_same_material_context: bool = False,
    ...
    for num_wann, material, source_task in tasks:
        ...
        if scoped_same_material_context:
            self_debug_records = scoped_self_debug_source_files(
                reports,
                candidate_reports,
                target_material=material,
                include_same_material_reports=include_same_material_reports,
            )
            bundle_writer = write_scoped_self_debug_context_bundle
        else:
            # Legacy path: preserve the original chemical-similarity bundle and
            # index byte-for-byte for the same inputs.
            self_debug_records = self_debug_source_files(
                reports,
                candidate_reports,
                include_same_material_reports=include_same_material_reports,
            )
            bundle_writer = write_self_debug_context_bundle
        ...
        next_run_context_text = ""
        if next_run_diagnoses_path is not None:
            next_run_context_text = next_run_recommendation_for_material(
                material,
                next_run_diagnoses_path,
            )
            if next_run_context_text:
                write_next_run_context_bundle(
                    target_task,
                    material=material,
                    diagnoses_path=next_run_diagnoses_path,
                    recommendation_text=next_run_context_text,
                )
                write_next_run_context_bundle(
                    target_task / "environment",
                    material=material,
        ...
        write_trace_tools(
            target_task / "environment",
            scoped_same_material_context=scoped_same_material_context,
        )
        ...
        if (
            next_run_context_text
            and "COPY next_run_context /app/next_run_context" not in dockerfile_text
        ):
            marker = "COPY material /app/material\n"
            if marker in dockerfile_text:
                dockerfile_text = dockerfile_text.replace(
                    marker,
                    marker + "COPY next_run_context /app/next_run_context\n",
                    1,
                )
            else:
                dockerfile_text += "\nCOPY next_run_context /app/next_run_context\n"
        ...
        instruction_builder = (
            scoped_context_instruction_appendix
            if scoped_same_material_context
            else context_instruction_appendix
        )
        instruction_text += instruction_builder(
            material,
            reports,
            candidate_reports,
            include_same_material_reports=include_same_material_reports,
        ...
        instruction_text += next_run_context_instruction_appendix(
            material,
            bool(next_run_context_text),
        )
...
def print_ordered_commands(args: argparse.Namespace, tasks: list[tuple[int, str, Path]]) -> None:
    ...
    for batch_start in range(0, len(tasks), batch_size):
        ...
        if prune_after_batch:
            ...
            for prune_command in DOCKER_PRUNE_COMMANDS:
                print(f"  {shlex.join(prune_command)} || overall_status=$?")
    print('  return "$overall_status"')
    print("}")
    print("run_harbor_num_wann_ordered")
...
def print_target_success_loop(args: argparse.Namespace, tasks: list[tuple[int, str, Path]]) -> None:
    ...
    print(f"DOCKER_PRUNE_COMMANDS = {DOCKER_PRUNE_COMMANDS!r}")
    print("")
    ...
    print("                try:")
    print("                    relative_parts = diagnostics_path.relative_to(root).parts")
    print("                    job_dir = root / relative_parts[0] if relative_parts else root")
    print("                except Exception:")
    print("                    job_dir = diagnostics_path.parents[2]")
    print("                task = TASK_BY_MATERIAL.get(material)")
    print("                if task is not None and self_debug_gate_passes(job_dir, task['task_dir']):")
    ...
    print("    next_index_path = task_path / 'environment' / 'next_run_context' / 'index.json'")
    print("    if not next_index_path.is_file():")
    print("        next_index_path = task_path / 'next_run_context' / 'index.json'")
    print("    if next_index_path.is_file():")
    print("        index_path = next_index_path")
    print("        trace_path = _latest_existing(job_path.rglob('next_run_file_trace.log'))")
    print("        summary_path = _latest_existing(job_path.rglob('NEXT_RUN_CONTEXT_SUMMARY.json'))")
    print("        verifier_path = task_path / 'environment' / 'verify_next_run_context_access.py'")
    print("        gate_label = 'next-run context'")
    print("    else:")
    ...
    print("        gate_label = 'self-debug'")
    print("    if not index_path.is_file() or trace_path is None or summary_path is None:")
    print("        print(f'    {gate_label} gate failed: missing index/trace/summary for {job_path}', flush=True)")
    print("        return False")
    print("    if not verifier_path.is_file():")
    print("        print(f'    {gate_label} gate failed: missing verifier script {verifier_path}', flush=True)")
    print("        return False")
    ...
    print("        return 0")
    print("    status = 0")
    print("    for command in DOCKER_PRUNE_COMMANDS:")
    print("        print('Pruning unused Docker resources after wave: ' + ' '.join(command), flush=True)")
    print("        result = subprocess.run(command).returncode")
    print("        if result != 0:")
    print("            status = result")
    print("    return status")
    print("")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "After each sorted batch finishes, run docker builder/system prune. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run docker builder/system prune after each material by requiring "
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Markdown file containing Codex-reviewed next-run diagnoses. When set, "
            "each generated task receives a material-specific "
            "/app/next_run_context bundle."
        ),
    )
    parser.add_argument(
    ...
    if args.next_run_diagnoses is not None:
        args.next_run_diagnoses = args.next_run_diagnoses.expanduser().resolve()
        if not args.next_run_diagnoses.is_file():
            raise SystemExit(f"--next-run-diagnoses does not exist: {args.next_run_diagnoses}")
    ...
    if tasks and not args.materials_only:
        augmented_dataset, tasks = materialize_self_debug_context_dataset(
            ...
            next_run_diagnoses_path=args.next_run_diagnoses,
        )
