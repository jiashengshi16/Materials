...
...
Self-debug context is staged through one scoped bundle. In
``self_review`` mode, that bundle contains prior reports for the current
material itself; no separate Codex recommendation Markdown is required.
"""
...
import generate_harbor_self_debug_context_commandNEW as self_debug_generator
...
# - "chemically similar": candidate-material self-debug reports selected from
#   include_only_candidates.csv are copied as context.
# - "self_review": prior self-debug reports for each current target material
#   are copied through the same bundle mechanism with scope="same_material".
WORKFLOW = "chemically similar"
SUPPORTED_WORKFLOWS = {"chemically similar", "self_review"}
...
DEFAULT_CANDIDATE_RUN_ERROR_TABLE = (
    self_debug_generator.ROOT
    / "include_only_candidatesIter2.csv"
)
DEFAULT_JOBS_ROOT = (
    self_debug_generator.ROOT / "jobsGeminiReviewsDeepseekIter3" / "ChemSimReruns2"
    if WORKFLOW == "self_review"
    else self_debug_generator.ROOT / "jobsGeminiReviewsDeepseekIter3"/ "ChemSimReruns2"
)
...
SAME_MATERIAL_CONTRADICTION_WARNING = """
## Warning about conflicting reports

These are independent reviews from different runs.

Reports with the same `scope` and report material may still contradict one
another. Reports from different materials or scopes may also differ because the
recipes, failure stages, and outcomes were different.

If there is a contradiction for the current strategy, inspect the individual
reports under `/app/self_debug_context/raw/` and decide which conclusion is
supported by the specific run evidence. In `self_review`, every report is
expected to have `scope: same_material` and to describe the current target
material itself.
...
CONTRADICTION_WARNING = (
    SAME_MATERIAL_CONTRADICTION_WARNING
    if WORKFLOW == "self_review"
    else LEGACY_CONTRADICTION_WARNING
)
...
def locked_runner_script(success_wave_timeout_sec: int = DEFAULT_SUCCESS_WAVE_TIMEOUT_SEC) -> str:
    """Return the common locked runner for every self-debug workflow."""
    return generic_locked_runner_script(success_wave_timeout_sec)

def legacy_locked_runner_instruction_appendix(material: str) -> str:
    return f"""
...
In the `self_review` workflow, first read the complete
...
def locked_runner_instruction_appendix(material: str) -> str:
    if WORKFLOW == "chemically similar":
        return legacy_locked_runner_instruction_appendix(material)

    if WORKFLOW == "self_review":
        context_contract = """
Before choosing the Wannierisation strategy, read the complete
`/app/self_debug_context/ALL_SELF_DEBUG_REPORTS.md` bundle and write
`workflow/SELF_DEBUG_CONTEXT_SUMMARY.json` exactly as required by the supplied
self-debug context instructions.

In this workflow every indexed report must have `scope: same_material`. Its
`report_material` and `target_material` must both identify the current task
material. These reports describe prior runs of the current material itself;
they are not chemically similar candidate-material reports and must not be
interpreted as evidence from a different material.
""".strip()
    else:
        context_contract = """
Before choosing the Wannierisation strategy, read the complete
`/app/self_debug_context/ALL_SELF_DEBUG_REPORTS.md` bundle and write
`workflow/SELF_DEBUG_CONTEXT_SUMMARY.json` exactly as required by the supplied
self-debug context instructions. Identify the `scope`, `report_material`, and
`target_material` of every report before deciding how directly its lessons
...
Use the original task instructions, the supplied files under `/app/material`,
and the copied self-debug context to decide the Wannierisation recipe yourself.
You may inspect compact metadata/log snippets, but keep terminal output small.
...
- The raw reviewed-run RMSE in eV is the absolute interpolation error for that
  prior run.
- The error ratio is `reviewed-run RMSE / reference_error_eV` for another
  evaluation on the same material. A ratio below 1 means the reviewed run beat
  that reference, a ratio near 1 means comparable performance, and a ratio
  above 1 means worse relative performance.
- For `scope: same_material`, use both metrics as direct evidence about prior
  runs of the current material, while still checking whether the current task
  files or contract differ.
- For `scope: candidate_material`, treat the report as cross-material context;
  do not transfer exact projections, band counts, windows, or final status
  without current-material evidence.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Legacy Codex recommendation Markdown option. It is ignored in the "
            "updated self_review workflow, which uses same-material "
            "self-debug reports from --self-debug-reviews-root."
        ),
    )
...
def install_locked_workflow_tools(
    tasks: list[tuple[int, str, Path]],
    ...
    for _num_wann, material, task_dir in tasks:
        ...
        # materialize_self_debug_context_dataset() already writes the standard
        # self-debug trace wrapper/verifier into this environment.
...
def deepseek_harbor_args(
    ...
    trace_wrapper_path = self_debug_generator.TRACE_WRAPPER_APP_PATH
    if WORKFLOW == "chemically similar":
        # Preserve the original command surface: the helper adds the self-debug
        # trace artifacts when no_gemini_file_trace is false.
        artifacts = list(CONTROLLED_ARTIFACTS)
    else:
        artifacts = list(
            dict.fromkeys(
                [*self_debug_generator.SELF_DEBUG_TRACE_ARTIFACTS, *CONTROLLED_ARTIFACTS]
            )
        )
...
def main() -> None:
    ...
    if WORKFLOW == "self_review":
        # Use the same scoped self-debug bundle as the chemically-similar
        # workflow, but populate it only with prior reports for the current
        # material itself.
        cli.include_candidate_self_debug_reports = False
        cli.candidate_self_debug_reports_only = False
        cli.next_run_diagnoses = None
    ...
    if not requested:
        if WORKFLOW == "self_review":
            raise SystemExit(
                ...
                f"or create same-material reports under {cli.self_debug_reviews_root}."
            )
        raise SystemExit(
            "No materials selected. Add names to MATERIALS, pass --material, "
    ...
    augmented_dataset, augmented_tasks = self_debug_generator.materialize_self_debug_context_dataset(
        ...
        next_run_diagnoses_path=None if WORKFLOW == "self_review" else cli.next_run_diagnoses,
        scoped_same_material_context=WORKFLOW == "self_review",
    )

    add_contradiction_warning(augmented_tasks)

    install_locked_workflow_tools(
        augmented_tasks,
